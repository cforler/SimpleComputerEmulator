...
public class Instruction implements Serializable {
    ...
    public String toCommand(){
        if(op == Opcode.NOOP || op == Opcode.HALT) return op.toString();
        String s = num? "#":"";
        return op +  " " + s  + operand;
    }
    ...
    public static String toBitString(int len, int value) {
        ...
        if(s.length() >= len) return s.substring(s.length()-len,s.length());
        
        StringBuffer zeros = new StringBuffer("");
        for(int i=s.length(); i < len; i++) zeros.append("0");
        return zeros + s;
    }
    ...
    public static Instruction fromBitString(String bitString) {
        ...
        if(bitString.length() != 16) return null;

        if( ! bitString.startsWith(Instruction.RESERVED)) return null;
        ...
        if( bitString.charAt(9) == '0') in.num = false;
        else if( bitString.charAt(9) == '1') in.num = true;
        else return null;
    ...
    public static boolean isComment(String cmd) {
        String s = cmd.trim().replaceAll("\\s+", " "); //Remove unneeded whitespaces
    ...
    private static String normalizeCommand(String cmd) {
        ...
        String s = cmd.trim().replaceAll("\\s+", " "); //Remove unneeded whitespaces
        if (Instruction.isComment(cmd)) return null;
        return s;
    }
    ...
    public static Instruction fromCommand(String cmd) {
        ...
        if(tmp.length == 1) {
            ...
            if(op == Opcode.NOOP) return new Instruction();
            if(op == Opcode.HALT) return new Instruction(Opcode.HALT ,true,0);
            return null;
        }
